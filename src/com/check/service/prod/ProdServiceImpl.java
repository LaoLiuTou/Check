 ...
 */ 
 ...
 public  Object addflowprod(Prod prod,String muladd,String termids,String subprod,String prod_assets){
	 ...
	 if(result>0){
		 if(prod_assets!=null&&!prod_assets.equals("")){
			 JSONArray prod_assetJA=JSONArray.fromObject(prod_assets);
			 for(int i=0;i<prod_assetJA.size();i++){
					JSONObject  addJO = (JSONObject) prod_assetJA.get(i);
					Prod_asset ps=new Prod_asset();
					if(addJO.containsKey("asset_id"))
						ps.setAsset_id(Long.parseLong(addJO.getString("asset_id")));
					if(addJO.containsKey("flag"))
						ps.setFlag(addJO.getString("flag"));
					ps.setBu_id(Long.parseLong(prod.getBu_id()));
					ps.setProd_id(prod.getId());
					iProd_assetMapper.addprod_asset(ps);
			 }
  ...
  */ 
 @Transactional
 public  Object updateflowprod(Prod prod,String muladd,String termids,String subprod,String prod_assets){
	 int result = 0;
	 ...
	 if(result>0){
		 ...
		 //删除prod_asset
		 iProd_assetMapper.deleteprod_assetbyprod(prod.getId()+"");
		 if(prod_assets!=null&&!prod_assets.equals("")){
			 JSONArray prod_assetJA=JSONArray.fromObject(prod_assets);
			 for(int i=0;i<prod_assetJA.size();i++){
					JSONObject  addJO = (JSONObject) prod_assetJA.get(i);
