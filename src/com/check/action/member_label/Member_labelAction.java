package com.check.action.member_label;
...
import com.check.service.member_label.IMember_labelService;
import com.check.model.member_label.Member_label;
public class Member_labelAction implements Action {
	private int page;
	private int size;
	...
	@Autowired
	private IMember_labelService iMember_labelService;
	private List<Member_label> list;
	private Member_label member_label;
	public int getPage() {
		return page;
	}
	...
	public Member_label getMember_label() {
		return member_label;
	}
	public void setMember_label(Member_label member_label) {
		this.member_label = member_label;
	}
	public List<Member_label> getList() {
		return list;
	}
	public void setList(List<Member_label> list) {
		this.list = list;
	}
	...
	public String add() throws Exception {
		...
		Member_label member_label =new Member_label(); 
		if(id!=null&&!id.equals(""))
		member_label.setId(Long.parseLong(id));
		if(c_dt!=null&&!c_dt.equals(""))
		member_label.setC_dt(sdf.parse(c_dt));
		if(up_dt!=null&&!up_dt.equals(""))
		member_label.setUp_dt(sdf.parse(up_dt));
		if(c_id!=null&&!c_id.equals(""))
		member_label.setC_id(Long.parseLong(c_id));
		if(labels_id!=null&&!labels_id.equals(""))
		member_label.setLabels_id(Long.parseLong(labels_id));
		if(members_id!=null&&!members_id.equals(""))
		member_label.setMembers_id(Long.parseLong(members_id));
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			int result = Integer.parseInt(iMember_labelService.addmember_label(member_label).toString());
			msg.append("\"success\",\"msg\":\"");
			msg.append(member_label.getId()+"\"");
	...
	public String list() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			list=iMember_labelService.selectmember_labelByParam(paramMap); 
			totalnumber=iMember_labelService.selectCountmember_labelByParam(paramMap);
	...
	public String update() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			iMember_labelService.updatemember_label(member_label);
			msg.append("\"success\",\"msg\":");
			msg.append("\"更新成功！\"");
			logger.info(id+"更新成功！");
		} catch (Exception e) {
	...
	public String delete() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			iMember_labelService.deletemember_label(id);
			msg.append("\"success\",\"msg\":");
			msg.append("\"删除成功！\"");
			logger.info(id+"删除成功！");
		} catch (Exception e) {
	...
	public String select() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			member_label=iMember_labelService.selectmember_labelById(id);
			JsonConfig jsonConfig = new JsonConfig();
			jsonConfig.registerJsonValueProcessor(Date.class, new JsonValueProcessor() {
				public Object processArrayValue(Object value, JsonConfig jsonConfig) {
					return value;  
			...
			msg.append("\"success\",\"msg\":");
			msg.append(JSONObject.fromObject(member_label, jsonConfig));
			logger.info(id+"查询成功！");
		} catch (Exception e) {
