package com.check.action.status;
...
import com.check.service.status.IStatusService;
...
public class StatusAction implements Action {
	...
	public String add() throws Exception {
		...
		status.setC_dt(sdf.parse(c_dt));
		if(up_dt!=null&&!up_dt.equals(""))
		status.setUp_dt(sdf.parse(up_dt));
		status.setCurr_status(curr_status);
		status.setStep_status(step_status);
		status.setType(type);
		if(sort!=null&&!sort.equals(""))
		status.setSort(Long.parseLong(sort));
		status.setFlag(flag);
	...
	public String list() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			list=iStatusService.selectstatusByParam(paramMap); 
			totalnumber=iStatusService.selectCountstatusByParam(paramMap);
	...
	public String select() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			status=iStatusService.selectstatusById(id);
			JsonConfig jsonConfig = new JsonConfig();
			jsonConfig.registerJsonValueProcessor(Date.class, new JsonValueProcessor() {
				public Object processArrayValue(Object value, JsonConfig jsonConfig) {
					return value;  
			...
			msg.append("\"success\",\"msg\":");
			msg.append(JSONObject.fromObject(status, jsonConfig));
			logger.info(id+"查询成功！");
		} catch (Exception e) {
