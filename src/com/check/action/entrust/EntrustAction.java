...
import com.check.service.entrust_asset.IEntrust_assetService;
...
import com.check.service.prod_asset.IProd_assetService;
...
public class EntrustAction implements Action {
	...
	@Autowired
	private IProd_assetService iProd_assetService;
	@Autowired
	private IEntrust_assetService iEntrust_assetService;
	...
	private String entrust_assets;
	
	public String getEntrust_assets() {
		return entrust_assets;
	}

	public void setEntrust_assets(String entrust_assets) {
		this.entrust_assets = entrust_assets;
	}
	...
	public String add() throws Exception {
		...
		try {
			int result = Integer.parseInt(iEntrustService.addentrust(entrust)
					.toString());
			...
			if (result > 0) {
				String qrResult = MatrixToImageWriter.createQrImage("TG_"
						+ entrust.getId());
	...
	public String remoteReview() throws Exception {
		response.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=UTF-8");
		...
		try {
			if (entrust_ids != null && !entrust_ids.equals("")) {
				...
				for (String e_id : e_ids) {
					...
					int e_snum = iEntrust_sampleService
							.selectCountentrust_sampleByParam(paramMap);
					paramMap.put("fromPage", 0);
					paramMap.put("toPage", e_snum);
					List<Entrust_sample> entrust_samples = iEntrust_sampleService
							.selectentrust_sampleByParam(paramMap);

					paramMap = new HashMap();
					paramMap.put("pid", e_id);
					// paramMap.put("up_dtFrom", sdf.parse(updatetime));
					int e_pinnum = iEntrust_pinService
							.selectCountentrust_pinByParam(paramMap);
					paramMap.put("fromPage", 0);
					paramMap.put("toPage", e_pinnum);
					List<Entrust_pin> entrust_pins = iEntrust_pinService
							.selectentrust_pinByParam(paramMap);

					iEntrustService.addRemoteFlowFunction(entrust,
							entrust_samples, entrust_pins);
				}
	...
	public String updateRemote() throws Exception {
		response.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=UTF-8");
		Entrust entrust = new Entrust();
		if (id != null && !id.equals(""))
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			iEntrustService.updateRemoteEntrustFlow(entrust, entrust_pins,entrust_assets);
			msg.append("\"success\",\"msg\":");
			msg.append("\"更新成功！\"");
			logger.info(id + "更新成功！");
		} catch (Exception e) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				if (sample != null) {
					...
					//状态为已取消停止新建
					if(sample.getSt_lv()!=null&&sample.getSt_lv().equals("已取消")){
						cancelIdList.add(sample.getId()+"");
					}
				}
				
			}

			if(cancelIdList.size()==0){
				// 分类
				...
				while (it.hasNext()) {
					...
					Pact pact = iPactService.selectpactById(templist.get(0)
							.getPid());
					...
					// 默认值
					entrust.setSt_lv("新建");
					entrust.setCjz_type("现场自助委托");
					entrust.setShow_flg("N");
					entrust.setCg_f(cg_f);
					entrust.setFlg("Y");
					entrust.setTy_lv("一般委托");
					entrust.setQy_lv("现场");

					Bu bu = iBuService.selectbuById(bu_id);
					if (bu != null) {
						entrust.setSp_id(bu.getApprove());
					}
					entrust.setPay_lv("未付款");
					// 常规价、复检价
					if (cg_f != null) {
						if (cg_f.equals("Y")) {
							entrust.setPrice(prod.getCgj());
						} else if (cg_f.equals("N")) {
							entrust.setPrice(prod.getFjj());
						}
					}
					//
					entrust.setCs_n(Long.parseLong(templist.size() + ""));
					// 更新委托单字段gg_code、sy_code
					List<String> gg_code = new ArrayList<String>();
					List<String> sy_code = new ArrayList<String>();
					for (Sample tempSample : templist) {
						gg_code.add(tempSample.getGg_t());
						sy_code.add(tempSample.getPart());
					}
					...
					for (String partStr : sy_code) {
						if (sy_code.indexOf(partStr) == 0) {
							...
							if (partStr.length() > 5) {
								sy_codeStr += partStr
										.substring(5, partStr.length()) + "、";
							}
						}

					}
					if (sy_codeStr.length() > 0) {
						sy_codeStr = sy_codeStr.substring(0,
								sy_codeStr.length() - 1);
					}
					...
					if(entrust.getJh_dt()==null||entrust.getJh_dt().equals("")){
						...
						gc.add(5, Integer.parseInt(prod.getZq_n() + ""));
						entrust.setJh_dt(sdf.format(gc.getTime()));
						// entrust.setSjwc_dt();
					}
				 

					Members tempMembers = iMembersService.selectmembersById(c_id);
					if (tempMembers != null) {
						if (tempMembers.getAccnt_id() != null) {
							Accnt accnt = iAccntService.selectaccntById(tempMembers
									.getAccnt_id() + "");
							entrust.setLq_id(accnt.getCont_id());
						}
					}
					...
					if (result > 0) {
						...
						List<Prod> prodList = iProdService
								.selectprodByParam(paramMap);
						for (Prod temp : prodList) {
							if (entrust.getCg_f() != null
									&& entrust.getCg_f().equals("Y")) {
								//
						...
						for (int i = 0; i < templist.size(); i++) {
							...
							iEntrust_sampleService
									.addentrust_sample(entrust_sample);
						}
						...
						// paramMap.put("up_dtFrom", sdf.parse(updatetime));
						int prod_assetnum = iProd_assetService.selectCountprod_assetByParam(paramMap);
						paramMap.put("fromPage", 0);
						paramMap.put("toPage", prod_assetnum);
						List<Prod_asset> tempList=iProd_assetService.selectprod_assetByParam(paramMap);
						for(Prod_asset temp :tempList){
							...
							iEntrust_assetService.addentrust_asset(entrust_asset);
						}
					...
					/*for (int i = 0; i < templist.size(); i++) {
						...
						// 更新样品
						int ennum = Integer.parseInt(sampleidJO.getString(sample
								.getId() + ""));
						for (int j = 0; j < ennum; j++) {
							sample.setId(null);
							// 编号
							sample.setPart(createSampleCode(sample.getBu_id(),
									prod.getId() + ""));
							// 批号
							sample.setLot(createSampleLot(sample.getBu_id(),
									sample.getSy_dt(), pact.getId() + "",
									prod.getId() + "", sample.getC_id()));
							sample.setSt_lv("已接收");

							int sresult = Integer.parseInt(iSampleService
									.addsample(sample).toString());
							if (sresult > 0) {
								String qrResult = MatrixToImageWriter
										.createQrImage("PS_" + sample.getId());
								if (qrResult.length() > 0) {
									HttpServletRequest request = ServletActionContext
											.getRequest();
									String path = request.getScheme() + "://"
				...
				msg.append("\"failure\",\"msg\":");
				msg.append("\""+StringUtils.join(cancelIdList,"|")+"\""); 
			}
			
			// msg.append(StringUtils.join(idList,"|")+"\"");
			/*
			 * } else{ msg.append("\"failure\",\"msg\":");
			 * msg.append("\"委托组数不能为空\""); }
			 */
		} catch (Exception e) {
			msg.append("\"failure\",\"msg\":");
	...
	public String select() throws Exception {
		...
		try {
			...
			if (entrust != null) {
				...
				if (tempPact != null)
					tempAccnt = iAccntService
							.selectaccntById(tempPact.getPid());
	...
	public String code() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			String appendCode=createAppend();
			msg.append("\"success\",\"msg\":\"");
			msg.append(appendCode + "\"");
			logger.info("获取列表成功！");
		} catch (Exception e) {
			msg.append("\"failure\",\"msg\":");
			msg.append("\"创建失败.\"");
			logger.info("创建追加编号！" + e);
			e.printStackTrace();
		}
		msg.append("}");
		if (callback == null) {
	...
	public String createAppend() {
		// http://192.168.1.144/Check/Entrust?pid=29&wt_dt=2017-11-10&bu_id=1&c_id=173
		String groupCode = pid + "_" + wt_dt;
		StringBuffer sb = new StringBuffer();
		Map paramMap = new HashMap();
		paramMap.put("fromPage", 0);
	...
	public String flow() throws Exception {
		...
		try {
			...
			int result = iEntrustService.entrustFlow(entrust, accnt, pact,
					sample, entrust_sample, test, results);
			if (result > 0) {
				String qrResult = MatrixToImageWriter.createQrImage("TG_"
						+ entrust.getId());
				if (qrResult.length() > 0) {
	...
	public String seFlow() throws Exception {
		...
		try {
			...
			// 编号
			/*
			 * if(flg.equals("Y")){ entrust.setCode(createCode()); } else{
			 * entrust.setCode(createAppend()); }
			 */
			...
			/*
			 * int result =
			 * iEntrustService.entrustFlow(entrust,accnt,pact,sample
			 * ,entrust_sample, test,results); if(result>0){ String qrResult =
			...
			if (sample != null && !sample.equals("")) {
				...
				List<String> resultList = iEntrustService.seFlow(entrust,
						sampleTree);
			...
			/*
			 * List<String> tableList = new ArrayList<String>();
			 * tableList.add("accnt"); tableList.add("lov");
			 * tableList.add("lot"); tableList.add("pact");
			 * CacheToRedis.cache(tableList);
			 */
		} catch (Exception e) {
			msg.append("\"failure\",\"msg\":");
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public String createSampleLot(String bu_id, String sy_dt, String pact_id,
			String prod_id, String c_id) {
		// http://192.168.1.144/Check/Sample?prod_id=14&bu_id=1&pid=194&sy_dt=2014-04-30
		// 00:00:00&c_id=11
		String groupcode = "";
		...
		if (lotList.size() > 0) {
			...
			if (lotnumberStr.equals("") || lotnumberStr.equals("null")) {
				...
				// String nextNum =
				// CodeUtils.autoGenericCode(lotnumberStr+"",3);
