package com.check.action.notice_member;
...
public class Notice_memberAction implements Action {
	private int page;
	private int size;
	...
	private List<Notice_member> list;
	private Notice_member notice_member;
	public int getPage() {
		return page;
	}
	...
	public Notice_member getNotice_member() {
		return notice_member;
	}
	public void setNotice_member(Notice_member notice_member) {
		this.notice_member = notice_member;
	}
	public List<Notice_member> getList() {
		return list;
	}
	public void setList(List<Notice_member> list) {
		this.list = list;
	}
	...
	public String add() throws Exception {
		...
		notice_member.setId(Long.parseLong(id));
		if(c_dt!=null&&!c_dt.equals(""))
		notice_member.setC_dt(sdf.parse(c_dt));
		if(up_dt!=null&&!up_dt.equals(""))
		notice_member.setUp_dt(sdf.parse(up_dt));
		if(c_id!=null&&!c_id.equals(""))
		notice_member.setC_id(Long.parseLong(c_id));
		if(notices_id!=null&&!notices_id.equals(""))
		notice_member.setNotices_id(Long.parseLong(notices_id));
		if(members_id!=null&&!members_id.equals(""))
		notice_member.setMembers_id(Long.parseLong(members_id));
		notice_member.setRead_flag(read_flag);
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			int result = Integer.parseInt(iNotice_memberService.addnotice_member(notice_member).toString());
			msg.append("\"success\",\"msg\":\"");
			msg.append(notice_member.getId()+"\"");
	...
	public String list() throws Exception {
		...
		paramMap.put("toPage",page*size); 
			...
			paramMap.put("read_flag", read_flag);
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			list=iNotice_memberService.selectnotice_memberByParam(paramMap); 
			totalnumber=iNotice_memberService.selectCountnotice_memberByParam(paramMap);
	...
	public String update() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			iNotice_memberService.updatenotice_member(notice_member);
			msg.append("\"success\",\"msg\":");
			msg.append("\"更新成功！\"");
			logger.info(id+"更新成功！");
		} catch (Exception e) {
	...
	public String delete() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			iNotice_memberService.deletenotice_member(id);
			msg.append("\"success\",\"msg\":");
			msg.append("\"删除成功！\"");
			logger.info(id+"删除成功！");
		} catch (Exception e) {
	...
	public String select() throws Exception {
		...
		StringBuffer msg = new StringBuffer("{\"state\":");
		try {
			notice_member=iNotice_memberService.selectnotice_memberById(id);
			JsonConfig jsonConfig = new JsonConfig();
			jsonConfig.registerJsonValueProcessor(Date.class, new JsonValueProcessor() {
				public Object processArrayValue(Object value, JsonConfig jsonConfig) {
					return value;  
			...
			msg.append("\"success\",\"msg\":");
			msg.append(JSONObject.fromObject(notice_member, jsonConfig));
			logger.info(id+"查询成功！");
		} catch (Exception e) {
